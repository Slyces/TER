...
def error(b, m, points):
    N = points.shape[0]
    return sum(map(lambda x: pow(x[0] - m*x[1] - b, 2), points)) / N
...
def gradient_step(b, m, points, learning_rate):
    """
...
if __name__ == '__main__':
    ...
    # Running the script
    run(args.data_file, m= args.m, b= args.b, lr= float(args.lr), n= int(args.n))
